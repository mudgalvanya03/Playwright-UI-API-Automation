...
export class ApiClient implements IApiClient{
    ...
    async get<T>(url: string, headers?: Record<string, string>):Promise<T>{
        ...
        const response = await this.request.get(`${this.apiURL}${url}`,{ headers} );
        if(!response.ok()){
    ...
    async put<T, B>(url: string, body: B, headers?: Record<string, string>):Promise<T> {
        logger.info("Sending Post Request", {url, body, method:'PUT'});
        const response= await this.request.put(`${this.apiURL}${url}`, { data: body, headers});
        if(!response.ok()){
    ...
    async post<T, B>(url: string, body: B, headers?: Record<string, string>):Promise<T>{
        ...
        const response= await this.request.post(`${this.apiURL}${url}`, { data: body, headers});
        if(!response.ok()){
            logger.error(`[API ERROR] ${url} | Status: ${response.status()}`);
    ...
    async patch<T, B>(url: string, body: B, headers?: Record<string,string>):Promise<T>{
        ...
        const response= await this.request.patch(`${this.apiURL}${url}`, { data: body, headers});
        if(!response.ok()){
            logger.error(`[API ERROR] ${url} | Status: ${response.status()}`);
    ...
    async delete<T>(url: string, headers?: Record<string, string>):Promise<T>{
        ...
        const response= await this.request.delete(`${this.apiURL}${url}`, { headers});
        if(!response.ok()){
            logger.error(`[API ERROR] ${url} | Status: ${response.status()}`);
